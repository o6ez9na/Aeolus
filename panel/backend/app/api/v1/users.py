...
async def create_user(body: UserCreate, session: SessionDep, _: AdminUser) -> User:
    user = User(
...
async def disable_user(user_id: uuid.UUID, session: SessionDep, admin: AdminUser) -> None:
    if user_id == admin.id:
